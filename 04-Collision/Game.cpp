...
*/
LPTEXTURE CGame::LoadTexture(LPCWSTR texturePath) // hàm dùng để load texture lên chương trình
{
...
void CGame::_ParseSection_SCENES(string line)
{
	...
	if (type == MAINSCENE) {
		...
		LPSCENE scene = new IntroGame(id, path);
		scenes[id] = scene;
	}
}
...
*/
void CGame::Load(LPCWSTR gameFile) // load game campaign lên
{
	...
	while (f.getline(str, MAX_GAME_LINE))
	{
		...
		if (line == "[SETTINGS]") { section = GAME_FILE_SECTION_SETTINGS; continue; } // scene mở đầu cho chương trình
...
void CGame::SwitchToMainScene(int scene_id, float start_x, float start_y)
{
	...
	((CPlayScene*)s)->PutPlayerIntoScene(mario);
